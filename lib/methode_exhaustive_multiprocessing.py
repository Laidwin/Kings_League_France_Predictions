...
def simulate_chunk(args) -> dict[Any | str, list[int]]:
    ...
    local_counter: dict[Any | str, list[int]] = {
        team: [0] * len(teams) for team in teams
    }

    # Simulation des scénarios
    match_indices: islice[tuple[int, ...]] = islice(
        product(range(len(match_outcomes)), repeat=len(remaining_matches)),
        start,
        start + end,
    )
...
def merge_counters(
    counters: list[dict[Any | str, list[int]]], teams: list[str]
) -> dict[str, list[int]]:
    """
...
def methode_exhaustive_multiprocessing(
    ...
    match_outcomes: list[tuple[int, int]] = [
        ...
        (1, 2),
        (0, 3),
    ]  # Victoire équipe1, nul équipe 1, nul équipe 2, victoire équipe2
    ...
    ranges: list[tuple[int, int]] = [
        (i * chunk_size, chunk_size) for i in range(num_cores)
    ]
    ...
    args_list: list[
        ...
        (start, end, current_points, remaining_matches, teams, match_outcomes)
