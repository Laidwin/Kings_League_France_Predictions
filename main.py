...
def main() -> None:
    ...
    choose: int = int(
        input(
            "Choisir la méthode de simulation (1: déterministe, 2: Monte Carlo, 3: exhaustive, 4: exhaustive multiprocessing): "
        )
    )
    ...
    if choose == 1:
        ...
        position_counter = methode_deterministe(
            nb_simulations, current_points, remaining_matches
        )
    elif choose == 2:
        print("Méthode Monte Carlo choisie")
        position_counter = monte_carlo(
            nb_simulations, current_points, remaining_matches
        )
    elif choose == 3:
        print("Méthode exhaustive choisie")
        position_counter, nb_simulations = methode_exhaustive(
            current_points, remaining_matches
        )
    elif choose == 4:
        print("Méthode exhaustive multiprocessing choisie")
        position_counter, nb_simulations = methode_exhaustive_multiprocessing(
            current_points, remaining_matches
        )
    else:
    ...
    for team in current_points.keys():
        probs: list[float] = [
            round((count / nb_simulations) * 100, 2) for count in position_counter[team]
        ]
        print(f"{team} : {probs}")
    ...
    df["Classement moyen"] = df.apply(
        lambda row: sum((i + 1) * p for i, p in enumerate(row)) / 100, axis=1
    )
    ...
    plt.title(
        "Simulation des classements - Kings League France",
        fontsize=72,
        weight="bold",
        pad=15,
    )
